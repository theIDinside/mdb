...
impl<'a> Section<'a> {
    pub fn from_object_file(section_index: usize, obj: &'a Object, section_header: &SectionHeader) -> Section<'a> {
        ...
        Section {
            section_index,
            section_type: section_header.segment_type,
...
impl SectionHeader {
    ...
    pub fn from_libc_repr(repr: &libc::Elf64_Shdr) -> SectionHeader {
        SectionHeader {
            ...
            section_data_offset: repr.sh_offset,
            size: repr.sh_size,
    ...
    /// Returns the address range where the data exists in the binary object file
...
impl std::fmt::Debug for SectionHeader {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SectionHeader")
            ...
            .field(
                "Object file offset",
                &format!("0x{:X}", &self.section_data_offset),
            )
